...
class ImageServiceRequest
  ...
  VALID_RESTRICTED_REQUEST_PARAMS = Set.new( %w(id filename format region zoom rotate))
  SIZE_CATEGORIES = %w(square thumb small medium large xlarge full)
  SIZE_REGEX = /_(#{SIZE_CATEGORIES.join('|')})$/i
  MAX_TILE_PIXELS = 512 * 512
  ...
  # @param [Boolean] restricted_request True if image is Stanford-only _thumb,_square, or tile request, false otherwise
  def initialize(params, restricted_request=false)  
    ...
    if(@restricted && restricted_params?(params))
      raise ArgumentError.new( "Image format and region are the only valid parameters for a restricted _thumb, _square, or tile request")
    end
    
    set_id(params[:id])
    set_filename(params[:filename])
    set_stacks_file_path()

    @resolver =  Djatoka::Resolver.new(DigiStacks.config.stacks.djatoka_url)
    @djatoka_region = @resolver.region(@stacks_file_path)

    set_file_format(params[:filename])
  ...
  # it will strip any extension with a known file type
  # @param [String] fname A filename that could have a file extension attached
  def set_filename(fname)
    if(format_in_filename?(fname))
  ...
  # @param [String] fname The file name you want to test
  # @return [Boolean] true if the extension of fname is a known mime type.  False otherwise
  def format_in_filename?(fname)
    !Rack::Mime.mime_type(File.extname(fname), nil).nil?
  end

  # stacks file path setter method
  def set_stacks_file_path()
...
#      raise ArgumentError.new( "Invalid filename parameter '#{file_prefix}' - does not exist in the digital stacks" )
  ...
  # file format setter method, default to jpg if the filename doesn't contain an extension
  # @param [String] fname Filename, which might include file extension
  def set_file_format(fname)
    if(format_in_filename?(fname))
      @format = File.extname(fname).split('.').last
    else
      @format = 'jpg'
    end
  end
  ...
  def get_file_prefix(fname)
    SIZE_CATEGORIES.each do |size|
      if( fname.match /_#{size}$/i )
        return fname.gsub(/_#{size}$/i,'')
      end
    end
    fname
  end
